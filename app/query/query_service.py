...
class QueryService:
  ...
  async def process_user_query(self, user_query: str, session_id: str, postures: list, current_posture_name, chat_history: list):
    ...
    update_argument = {"$push": {"chat_history.messages": {"human": user_query, "ai": llm_response_text, "message_id": llm_response_message_id}}}
  ...
  async def process_transition_query(self, transition_from_idx: int, session_id: str, postures: list, chat_history: list):
    ...
    update_argument = {
        "$push": {"chat_history.messages": {"human": "Please narrate the transition from current posture to the next one", "ai": llm_response_text, "message_id": llm_response_message_id}},
